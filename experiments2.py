...
def run_policies(setting_name, policy_name, init_method, T):
    ...
    for i,N in enumerate(Ns):
        for rep in range(num_reps):
            ...
            if policy_name == "id":
                ...
                for t in range(T):
                    cur_states = rb.get_states()
                    focus_set, non_shrink_flag = policy.get_new_focus_set(cur_states=cur_states, last_focus_set=focus_set)
                ...
                policy = FTVAPolicy(setting.sspa_size, setting.trans_tensor, setting.reward_tensor, y=y, N=N, act_frac=act_frac, init_virtual=None)
                for t in range(T):
            ...
            print("setting={}, policy={}, N={}, rep_id={}, avg reward = {}".format(setting_name, policy_name, N, rep, avg_reward))
...
def figure_from_multiple_files():
    settings = ["eight-states"] #["eight-states", "three-states"]  #["eight-states", "three states", "non-sa"]
    policies = ["id", "setexp", "setopt", "ftva", "lppriority"] # ["id", "setexp", "setopt", "ftva", "lppriority"]
    reward_array_dict = {}
    ...
    for setting_name in settings:
        for policy_name in policies:
            if (policy_name in ["id", "setexp", "setopt"]) or \
                    ((setting_name, policy_name) in [("non-sa", "ftva"), ("non-sa", "lppriority")]):
                with open("fig_data/{}-{}-N{}-{}-{}".format(setting_name, policy_name, 100, 1000, "same"), 'rb') as f:
    ...
    for setting_name in settings:
        if setting_name == "eight-states":
            ...
            plt.plot(Ns, np.array([0.12380016733626052]*len(Ns)), label="upper bound", linestyle="--")
        elif setting_name == "non-sa":
            plt.plot(Ns, np.array([1]*len(Ns)), label="upper bound", linestyle="--")
        for policy_name in policies:
            plt.plot(Ns, np.average(reward_array_dict[(setting_name, policy_name)], axis=0), label=policy_name, linewidth=1.5)
...
if __name__ == "__main__":
    # for setting_name in ["eight-states"]:   #["eight-states", "three-states", "non-sa"]:
    #     for policy_name in ["id", "setexp", "setopt"]:
    #         run_policies(setting_name, policy_name, "same", 10000)
    figure_from_multiple_files()
    # run_policies("eight-states", "id", "same", 1000)
