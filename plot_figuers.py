...
def figure_from_multiple_files():
    ...
    settings = ["non-sa", "non-sa-big2"]
    policies = ["whittle", "lppriority", "ftva", "setexp", "setexp-priority", "id"]
    skip_policies = ["whittle", "lppriority", "setexp", "setexp-priority", "id"]# ["setexp", "setexp-priority"]
    linestyle_str = ["-.", "-", "--", "-.", "--", "-"]
    ...
    for setting_name, policy_name, cur_init_method in setting_policy_init_tuples:
        ...
        print(file_prefix)
        file_dir = "fig_data"
        if cur_init_method not in ["bad", "given"]: ## todo: comment this if condition out
        ...
        for i in range(len(Ns)):
            assert len(batch_means_dict[(setting_name, policy_name, cur_init_method)][i]) == target_num_batches
    ...
    for setting_name in settings:
        ...
        plt.savefig("figs2/{}-N{}-{}-hybrid.pdf".format(setting_name, Ns[0], Ns[-1]))
        ## todo: uncomment this
